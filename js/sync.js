...
(function () {
  ...
  // each full row on its own, one at a time. A bounded-concurrency version
  // of this loop was tried and reverted — it didn't reliably load every
  // page in practice, and this whole area of the code has already caused
  // one real data-loss incident from being "clever" about concurrency;
  // slow-but-correct wins until there's a solid, well-tested reason to
  // revisit it.
      ...
      rows.forEach(function (row) {
        chain = chain.then(function () {
          return client.from("library_pages").select("ficha").eq("id", row.id).maybeSingle().then(function (full) {
            ...
            // Missing on a page saved before this fix — its ficha never
            // had its images pulled out to begin with (still several MB
            // inline, in whatever this row's own `ficha` column already
            // held above).
  ...
  // into one query AND firing several at once turned out unreliable in
  // practice (see loadLibraryRemote's own note on the same thing, one
  // level up). Slow-but-correct over clever-but-flaky here.
